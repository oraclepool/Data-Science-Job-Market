...
*/
(function(b) {
    "object" === typeof module && module.exports ? (b["default"] = b, module.exports = b) : "function" === typeof define && define.amd ? define("highcharts/modules/variable-pie", ["highcharts"], function(g) {
        b(g);
        b.Highcharts = g;
        return b
    ...
    function g(b, d, g, n) {
        b.hasOwnProperty(d) || (b[d] = n.apply(null, g))
    }
    b = b ? b._modules : {};
    g(b, "modules/variable-pie.src.js", [b["parts/Globals.js"], b["parts/Utilities.js"]], function(b, d) {
        var g = d.arrayMax,
            n = d.arrayMin,
            w = d.clamp,
            x = d.fireEvent,
            p = d.pick;
        d = d.seriesType;
        var y = b.seriesTypes.pie.prototype;
        d("variablepie", "pie", {
            minPointSize: "10%",
            maxPointSize: "100%",
            zMin: void 0,
            ...
            calculateExtremes: function() {
                var a = this.chart,
                    b = this.options;
                var c = this.zData;
                var d = Math.min(a.plotWidth, a.plotHeight) - 2 * (b.slicedOffset || 0),
                    l = {};
                ...
                c.length && (a = p(b.zMin, n(c.filter(this.zValEval))), c = p(b.zMax, g(c.filter(this.zValEval))), this.getRadii(a, c, this.minPxSize,
                    this.maxPxSize))
            },
            getRadii: function(a, b, c, d) {
                var l = 0,
                    ...
                    u = b - a;
                for (l; l < q; l++) {
                    var h = this.zValEval(g[l]) ? g[l] : a;
                    h <= a ? h = c / 2 : h >= b ? h = d / 2 : (h = 0 < u ? (h - a) / u : .5, p && (h = Math.sqrt(h)), h = Math.ceil(c + h * (d - c)) / 2);
                    k.push(h)
                }
                this.radii = k
            },
            translate: function(a) {
                ...
                for (k = 0; k < h; k++) {
                    ...
                    f.slicedTranslation = {
                        translateX: Math.round(Math.cos(e) * d),
                        translateY: Math.round(Math.sin(e) * d)
                    };
                    var v = Math.cos(e) * a[2] / 2;
                    var w = Math.sin(e) * a[2] / 2;
                    r = Math.cos(e) * m;
                    m *= Math.sin(e);
                    f.tooltipPos = [a[0] + .7 * v, a[1] + .7 * w];
                    f.half = e < -Math.PI / 2 || e > Math.PI / 2 ? 1 : 0;
                    f.angle = e;
                    v = Math.min(g, f.labelDistance / 5);
                    f.labelPosition = {
                        natural: {
                            x: a[0] + r + Math.cos(e) * f.labelDistance,
                            y: a[1] + m + Math.sin(e) * f.labelDistance
                        },
                        "final": {},
                        alignment: f.half ? "right" : "left",
                        connectorPosition: {
                            breakAt: {
                                x: a[0] + r + Math.cos(e) * v,
